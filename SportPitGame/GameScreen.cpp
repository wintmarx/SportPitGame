...
void GameScreen::Render(Graphics *graphics)
{
	...
	{
	graphics->DrawLine(0, i * fieldSize, cellsCount * fieldSize, i * fieldSize, &glm::vec4(1, 0, 0, 1), &(projection*view));
	}
	for (int j = 0; j < cellsCount + 1; j++)
	{
	graphics->DrawLine(j * fieldSize, 0, j * fieldSize, cellsCount * fieldSize, &glm::vec4(1, 0, 0, 1), &(projection*view));
	}
